...
# -*- coding: utf-8 -*-
"""
webhook_app.py — v4.4.3
- Fluxo estrito, GEN_AUTO por ENV, anti-tilt e anti-repetição imediata
- Timeout de fechamento DESATIVADO (fecha só quando vier fechamento real)
- Parsers compatíveis com:
  * ENTRADA CONFIRMADA … Sequência: 4 | 3 | 2 | 3 … Entrar após o 4
  * 💰 APOSTA ENCERRADA … RED ❌ (1 | 1 | 1) / GREEN ✅ (3 | 2 | 4)
  * ANALISANDO … Sequência: 4 | 3 | 4
- Placar diário com reset às 00:00 (TZ_NAME)
- DEBUG de texto do fonte (mostra uma prévia da mensagem recebida)
...
GEN_AUTO       = os.getenv("GEN_AUTO", "1").strip().lower() in ("1","true","yes")
...
# ========= App =========
app = FastAPI(title="guardiao-auto-bot (GEN webhook)", version="4.4.3")

# ========= Parâmetros =========
DECAY = 0.980
W4, W3, W2, W1 = 0.42, 0.30, 0.18, 0.10
OBS_TIMEOUT_SEC = 10**9  # desativado
COOLDOWN_N = 2
K_SHORT, K_LONG = 60, 300
...
# ========= N-gram / Feedback / Probabilidades =========
...
# LLM opcional (seguro para ficar vazio; não quebra)
def _llm_probs_from_tail(_tail:List[int])->Dict[int,float]:
    return {}  # mantenha vazio para não depender de modelo local

def _blend(p1,p2,p3,p4):
    w1=w2=w3=w4=1.0; S=(w1+w2+w3+w4); w1=w1/S; w2=w2/S; w3=w3/S; w4=w4/S
    b={c:w1*p1.get(c,0)+w2*p2.get(c,0)+w3*p3.get(c,0)+w4*p4.get(c,0) for c in (1,2,3,4)}
    s=sum(b.values()) or 1e-9; return {k:v/s for k,v in b.items()}

def _post_from_tail(tail:List[int], after:Optional[int])->Dict[int,float]:
    # contexto baseado em n-gram/feedback + frequências
    cands=[1,2,3,4]
    ...
    base={c:0.0 for c in cands}
    if len(ctx4)==4: 
        for c in cands: base[c]+=W4*_prob_from_ngrams(ctx4[:-1],c)
    if len(ctx3)==3: 
        for c in cands: base[c]+=W3*_prob_from_ngrams(ctx3[:-1],c)
    if len(ctx2)==2: 
        for c in cands: base[c]+=W2*_prob_from_ngrams(ctx2[:-1],c)
    if len(ctx1)==1: 
        for c in cands: base[c]+=W1*_prob_from_ngrams(ctx1[:-1],c)
    # feedback
    if len(ctx4)==4:
        for c in cands: base[c]+=0.45*W4*_feedback_prob(4,ctx4[:-1],c)
    if len(ctx3)==3:
        for c in cands: base[c]+=0.45*W3*_feedback_prob(3,ctx3[:-1],c)
    if len(ctx2)==2:
        for c in cands: base[c]+=0.45*W2*_feedback_prob(2,ctx2[:-1],c)
    if len(ctx1)==1:
        for c in cands: base[c]+=0.45*W1*_feedback_prob(1,ctx1[:-1],c)
    base=_norm(base)
    return _blend(base, _post_freq_k(tail,K_SHORT), _post_freq_k(tail,K_LONG), _llm_probs_from_tail(tail) or {1:0.25,2:0.25,3:0.25,4:0.25})
...
def _streak_adjust_choice(post:Dict[int,float], gap:float, ls:int)->Tuple[int,str,Dict[int,float]]:
    ...
    if ls>=3:
        comp=_norm({c:max(1e-9,1.0-post[c]) for c in (1,2,3,4)})
        post=_norm({c:0.7*post[c]+0.3*comp[c] for c in (1,2,3,4)})
        ranking=sorted(post.items(), key=lambda kv: kv[1], reverse=True); best=ranking[0][0]; reason="IA_anti_tilt_mix"
    if ls>=2 and len(ranking)>=2:
        gap0=ranking[0][1]-ranking[1][1]
        if gap0<0.05: best=ranking[1][0]; reason="IA_runnerup_ls2"
...
def choose_single_number(after:Optional[int]):
    ...
    best=ranking[0][0]; conf=float(post[best]); ls=_get_loss_streak()
    best,reason,post_adj=_streak_adjust_choice(post,gap,ls)
    conf=float(post_adj[best]); r2=sorted(post_adj.items(), key=lambda kv: kv[1], reverse=True)[:2]
...
def parse_close_numbers(text: str):
    ...
    if not CLOSE_TAG_RX.search(t): return []
    mg=CLOSE_NUMS_RX.findall(t)
    if mg:
        last=mg[-1]; nums=re.findall(r"[1-4]", _normalize_keycaps(last))
        return [int(x) for x in nums][:3]
    nums=re.findall(r"[1-4]", t); return [int(x) for x in nums][:3]

def parse_analise_seq(text: str):
    t=_normalize_keycaps(text or "")
    if not ANALISANDO_RX.search(t): return []
...
def _seen_list(row: sqlite3.Row) -> List[str]:
    seen=(row["seen"] or "").strip(); return [s for s in seen.split("-") if s]
...
def _close_with_outcome(row: sqlite3.Row, outcome: str, final_seen: str, stage_lbl: str, suggested: int):
    with _tx() as con: con.execute("UPDATE pending SET open=0, seen=? WHERE id=?", (final_seen, int(row["id"])))
    ...
    try:
        obs_add=[int(x) for x in final_seen.split("-") if x.isdigit()]; append_seq(obs_add)
    except Exception: pass
    ...
    return (f"{'🟢' if outcome.upper()=='GREEN' else '🔴'} <b>{outcome.upper()}</b> — finalizado "
            ...
            f"📊 Geral: {score_text()}\n\n{snapshot}")
def _maybe_close_by_timeout(): return None  # desativado
...
async def webhook(token: str, request: Request):
    ...
    try:
        ...
        # DEBUG: mostre o texto que chegou do fonte
        if DEBUG_MSG:
            preview = (text or "").replace("\n", " ⏎ ")[:180]
            await tg_send_text(TARGET_CHANNEL, f"DEBUG fonte={chat_id} texto='{preview}'")

        # Filtro do canal fonte
        if SOURCE_CHANNEL and not BYPASS_SOURCE and chat_id != str(SOURCE_CHANNEL):
            return {"ok": True, "skipped": "outro_chat"}
        if not text:
            return {"ok": True, "skipped": "sem_texto"}

        norm = _normalize_keycaps(text)

        # ANALISANDO
        if ANALISANDO_RX.search(norm):
            seq = parse_analise_seq(text)
            if seq: append_seq(seq)
            return {"ok": True, "analise_seen": len(seq)}

        # FECHAMENTO
        if CLOSE_TAG_RX.search(norm):
            pend = get_open_pending()
            if pend:
        ...
        if best == last_sug:
            ...
            best = alt; conf = float(post.get(best, conf)); reason = (reason + "_avoid_repeat")[:64]
        ...
        if DEBUG_MSG:
            try: await tg_send_text(TARGET_CHANNEL, f"⚠️ EXCEPTION no /webhook: <code>{type(e).__name__}: {str(e)[:400]}</code>")
            except Exception: pass
