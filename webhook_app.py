...
def _fmt_seq_conferencia(seen_csv: str, hit: int, _esperado: int) -> str:
    """
    Pega até 2 últimos números observados e marca o que saiu agora entre parênteses.
    Ex.: seen='2,4' e hit=1  -> '2 | 4 | (1)'
    """
    arr = [int(x) for x in (seen_csv or "").split(",") if x]
    arr = arr[-2:]  # dois últimos
...
async def send_green_imediato(sugerido:int, stage_txt:str="G0", seq_txt: str = ""):
    """
    NOVO: mostra o número que bateu entre parênteses + linha de conferência logo abaixo.
    """
...
async def send_loss_imediato(sugerido:int, stage_txt:str="G0", obs: Optional[int] = None, seq_txt: str = ""):
    """
    NOVO: no LOSS, mostra o número observado entre parênteses + linha de conferência.
    """
