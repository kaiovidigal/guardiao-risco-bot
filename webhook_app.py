...
# -*- coding: utf-8 -*-
"""
webhook_app.py — v5.0 (G6 + especialistas intactos)
- Fluxo estrito + Anti-tilt (sem reduzir sinais) + robustez de canal
- IA local (LLM) como 4º especialista (opcional)
- "ANALISANDO": aprende sequência, pode abrir e pode adiantar/fechar
- Placar zera todo dia às 00:00 (fuso TZ_NAME, default America/Sao_Paulo)
- CHAIN_ON: após fechar, abre automaticamente novo sinal
- (NOVO) Suporte até G6 (G0..G6) sem quebrar o fluxo original
...
# ---- GALES (G0..G6) ----
MAX_GALES = 6                    # G0 (entrada), G1..G6
MAX_OBS = 1 + MAX_GALES          # total de observações possíveis
OBS_TIMEOUT_SEC = 240            # fecha por timeout (se já houver progresso)
...
import httpx
app = FastAPI(title="guardiao-auto-bot (GEN webhook)", version="5.0")

# ========= Parâmetros do modelo =========
DECAY = 0.980
W4, W3, W2, W1 = 0.42, 0.30, 0.18, 0.10

# ======== Gates (não bloqueiam abertura; mantidos) ========
CONF_MIN    = 0.70
GAP_MIN     = 0.13
H_MAX       = 0.85
FREQ_WINDOW = 90
...
def parse_close_numbers(text: str) -> List[int]:
    """Extrai até MAX_OBS números (1..4)."""
...
def parse_analise_seq(text: str) -> List[int]:
    ...
    if not mseq:
        return []
    return [int(x) for x in re.findall(r"[1-4]", mseq.group(1))][:MAX_OBS]
...
def _stage_from_observed(suggested: int, obs: List[int]) -> Tuple[str, str]:
    """
    Retorna (outcome, stage_lbl).
    G0: obs[0]==sug; G1: obs[1]==sug; ...; G6: obs[6]==sug. Caso contrário, LOSS.
    """
    for i in range(min(len(obs), MAX_OBS)):
        if obs[i] == suggested:
            return ("GREEN", f"G{i}")
...
def _close_with_outcome(row: sqlite3.Row, outcome: str, final_seen: str, stage_lbl: str, suggested: int):
    ...
    try:
        ...
        if outcome.upper() == "GREEN":
            stage_index = int(stage_lbl[1:]) if stage_lbl.startswith("G") and stage_lbl[1:].isdigit() else 0
            # pesos decrescentes por profundidade (G0>G1>...>G6)
            stage_weight = [1.00, 0.75, 0.55, 0.45, 0.38, 0.32, 0.28]
            stage_weight = stage_weight[stage_index] if stage_index < len(stage_weight) else 0.25
...
def _maybe_close_by_timeout():
    """
    Se passou muito tempo e já houve progresso (>=2 observados),
    completa com 'X' até MAX_OBS e fecha.
    """
    ...
    if len(seen_list) >= 2 and len(seen_list) < MAX_OBS:
...
async def health():
    ...
    return {"ok": True, "db": DB_PATH, "pending_open": pend_open, "pending_seen": seen, "time": ts_str(), "tz": TZ_NAME, "max_gales": MAX_GALES}
...
async def webhook(token: str, request: Request):
    ...
    # 1) Gales (informativo rápido — mantém compatibilidade com mensagens do fonte)
    ...
    if GREEN_RX.search(text) or LOSS_RX.search(text):
        pend = get_open_pending()
        if pend:
            nums = parse_close_numbers(text)  # agora até MAX_OBS
            if nums:
