# -*- coding: utf-8 -*-
# Fan Tan — Guardião (G0 + Recuperação G1/G2) — IA bigrama
#
# Destaques:
# - IA baseada em BIGRAMA (+ apoio de unigram), com warm-up (exploração) e thresholds dinâmicos.
# - GREEN/RED: salva o ÚLTIMO número entre parênteses. ANALISANDO só registra sequência.
# - Recuperação G1/G2 separada (placar mostra G0, G1, G2 e Loss).
...
#
# Requisitos ENV:
#   TG_BOT_TOKEN, WEBHOOK_TOKEN, PUBLIC_CHANNEL(opcional), REPL_CHANNEL(opcional)
#
...
PUBLIC_CHANNEL = os.getenv("PUBLIC_CHANNEL", "").strip()   # não usado para broadcast (apenas réplica)
WEBHOOK_TOKEN  = os.getenv("WEBHOOK_TOKEN", "").strip()
FLUSH_KEY      = os.getenv("FLUSH_KEY", "meusegredo123").strip()

# Canal de réplica (onde NOSSO sinal é postado)
REPL_CHANNEL   = os.getenv("REPL_CHANNEL", "-1003052132833").strip()  # @fantanautomatico2
REPL_ENABLED   = True
...
FAST_RESULT   = True          # manda G0 assim que encontrar
SCORE_G0_ONLY = False         # ⬅️ Contabiliza G0/G1/G2 separadamente

# IA com ênfase em BIGRAMA + UNIGRAMA
WINDOW = 400
DECAY  = 0.985
# Pesos focados em bigrama:
W4, W3, W2, W1 = 0.00, 0.00, 0.70, 0.30
ALPHA, BETA, GAMMA = 1.05, 0.70, 0.40
GAP_MIN = 0.08

# Filtros base (ajustados dinamicamente pela fase + warm-up)
MIN_CONF_G0 = 0.55
MIN_GAP_G0  = 0.04
MIN_SAMPLES = 1000
...
app = FastAPI(title="Fantan Guardião — FIRE-only (G0 + Recuperação G1/G2)", version="4.0.0")
...
def init_db():
    ...
    # daily_score_ia pode estar em versão antiga (sem g1/g2); garantir colunas
    for col in ["g1","g2"]:
        try: cur.execute(f"ALTER TABLE daily_score_ia ADD COLUMN {col} INTEGER NOT NULL DEFAULT 0"); con.commit()
        except sqlite3.OperationalError: pass
    con.close()
...
async def ia2_send_signal(best:int, conf:float, tail_len:int, mode:str):
    ...
    txt = (f"🤖 <b>{SELF_LABEL_IA} [{mode}]</b>\n"
           ...
           f"📈 Conf: <b>{conf_capped*100:.2f}%</b> | Amostra≈<b>{tail_len}</b>")
...
def update_daily_score_ia(stage: Optional[int], won: bool):
    y = today_key_local()
    row = query_one("SELECT g0,g1,g2,loss,streak FROM daily_score_ia WHERE yyyymmdd=?", (y,))
    if not row: g0=g1=g2=loss=streak=0
    else:       g0,g1,g2,loss,streak = row["g0"],row["g1"],row["g2"],row["loss"],row["streak"]
    ...
    exec_write("""
      INSERT INTO daily_score_ia (yyyymmdd,g0,g1,g2,loss,streak)
      VALUES (?,?,?,?,?,?)
      ON CONFLICT(yyyymmdd) DO UPDATE SET
        g0=excluded.g0, g1=excluded.g1, g2=excluded.g2, loss=excluded.loss, streak=excluded.streak
    """, (y,g0,g1,g2,loss,streak))
...
def _convert_last_loss_to_green_stage(stage:int):
    """Converte 1 LOSS em GREEN no estágio indicado (G1 ou G2)."""
    y = today_key_local()
    row = query_one("SELECT g0,g1,g2,loss,streak FROM daily_score WHERE yyyymmdd=?", (y,))
    if not row:
        exec_write("""INSERT OR REPLACE INTO daily_score (yyyymmdd,g0,g1,g2,loss,streak) VALUES (?,0,0,0,0,0)""",(y,))
        return
    g0=row["g0"] or 0; g1=row["g1"] or 0; g2=row["g2"] or 0; loss=row["loss"] or 0; streak=row["streak"] or 0
    if loss>0: loss -= 1
    if stage==1: g1 += 1
    elif stage==2: g2 += 1
    streak += 1
...
def _convert_last_loss_to_green_ia_stage(stage:int):
    y = today_key_local()
    row = query_one("SELECT g0,g1,g2,loss,streak FROM daily_score_ia WHERE yyyymmdd=?", (y,))
    if not row:
        exec_write("""INSERT OR REPLACE INTO daily_score_ia (yyyymmdd,g0,g1,g2,loss,streak) VALUES (?,0,0,0,0,0)""",(y,))
        return
    g0=row["g0"] or 0; g1=row["g1"] or 0; g2=row["g2"] or 0; loss=row["loss"] or 0; streak=row["streak"] or 0
    if loss>0: loss -= 1
    if stage==1: g1 += 1
    ...
    streak += 1
    exec_write("""INSERT OR REPLACE INTO daily_score_ia (yyyymmdd,g0,g1,g2,loss,streak) VALUES (?,?,?,?,?,?)""",
               (y,g0,g1,g2,loss,streak))

def _ia_acc_days(days:int=7) -> Tuple[int,int,float]:
    days = max(1, min(int(days), 30))
    rows = query_all("""SELECT g0,g1,g2, loss FROM daily_score_ia ORDER BY yyyymmdd DESC LIMIT ?""",(days,))
    g = sum((r["g0"] or 0)+(r["g1"] or 0)+(r["g2"] or 0) for r in rows)
...
IA2_GAP_SAFETY              = 0.08
IA2_DELTA_GAP               = 0.03
IA2_MAX_PER_HOUR            = 20   # limite de velocidade/hora
IA2_COOLDOWN_AFTER_LOSS     = 10   # cooldown menor p/ velocidade
IA2_MIN_SECONDS_BETWEEN_FIRE= 6    # espaçamento mínimo (rápido)

_S_A_MAX=20_000; _S_B_MAX=100_000
_ACC_TO_B=0.55; _ACC_TO_C=0.58; _ACC_FALL_B=0.52; _ACC_FALL_C=0.55
_PHASE_DWELL=60*60
_phase_name, _phase_since_ts = "A", 0

def _current_samples() -> int:
    row = query_one("SELECT SUM(weight) AS s FROM ngram_stats"); return int((row["s"] or 0) if row else 0)
...
def _adaptive_phase() -> str:
    ...
    s=_current_samples(); _g,_l,acc7=_ia_acc_days(7); target=_decide_phase(s,acc7)
    now=now_ts()
    if _phase_since_ts==0: _phase_name, _phase_since_ts = target, now; return _phase_name
...
def _eff() -> dict:
    p=_adaptive_phase()
    if p=="A":
        return {"PHASE":"A","MAX_CONCURRENT_PENDINGS":2,"IA2_MAX_PER_HOUR":IA2_MAX_PER_HOUR,
    ...
    if p=="B":
        return {"PHASE":"B","MAX_CONCURRENT_PENDINGS":2,"IA2_MAX_PER_HOUR":IA2_MAX_PER_HOUR,
                "IA2_MIN_SECONDS_BETWEEN_FIRE":IA2_MIN_SECONDS_BETWEEN_FIRE+2,
                "IA2_COOLDOWN_AFTER_LOSS":IA2_COOLDOWN_AFTER_LOSS+2,
                "MIN_CONF_G0":0.53,"MIN_GAP_G0":0.035}
    return {"PHASE":"C","MAX_CONCURRENT_PENDINGS":1,"IA2_MAX_PER_HOUR":IA2_MAX_PER_HOUR//2,
            "IA2_MIN_SECONDS_BETWEEN_FIRE":IA2_MIN_SECONDS_BETWEEN_FIRE+4,
            "IA2_COOLDOWN_AFTER_LOSS":IA2_COOLDOWN_AFTER_LOSS+5,
...
def _daily_score_snapshot():
    y=today_key_local()
    row=query_one("SELECT g0,g1,g2,loss,streak FROM daily_score WHERE yyyymmdd=?", (y,))
    if not row: return 0,0,0,0,0.0,0
    g0=row["g0"] or 0; g1=row["g1"] or 0; g2=row["g2"] or 0; loss=row["loss"] or 0; streak=row["streak"] or 0
...
def _daily_score_snapshot_ia():
    y=today_key_local()
    row=query_one("SELECT g0,g1,g2,loss,streak FROM daily_score_ia WHERE yyyymmdd=?", (y,))
    g0=(row["g0"] if row else 0); g1=(row["g1"] if row else 0); g2=(row["g2"] if row else 0); loss=(row["loss"] if row else 0)
    total=g0+g1+g2+loss; acc=( (g0+g1+g2)/total*100.0 ) if total else 0.0
    return g0,g1,g2,loss,acc

def _last_snapshot_info():
    try:
        p=os.path.join(INTEL_DIR,"snapshots","latest_top.json")
        if not os.path.exists(p): return "—"
        ts=os.path.getmtime(p); return datetime.utcfromtimestamp(ts).replace(tzinfo=timezone.utc).isoformat()
    except: return "—"
...
def _health_text() -> str:
    ...
    g0,g1,g2,loss,acc,streak=_daily_score_snapshot()
    ia_g0,ia_g1,ia_g2,ia_loss,ia_acc=_daily_score_snapshot_ia()
    try: last_reason_age=max(0, now_ts()-(_ia2_last_reason_ts or now_ts())); last_line=f"🤖 Motivo último NÃO-FIRE/FIRE: {_ia2_last_reason} (há {last_reason_age}s)"
    except: last_line="🤖 Motivo último NÃO-FIRE/FIRE: —"
    try:
        eff=_eff(); _g7,_l7,acc7=_ia_acc_days(7)
        phase_line=(f"🧭 Fase: {eff['PHASE']} | Max/h: {eff['IA2_MAX_PER_HOUR']} | Conc.: {eff['MAX_CONCURRENT_PENDINGS']} | "
                    f"Conf≥{eff['MIN_CONF_G0']:.2f}/Gap≥{eff['MIN_GAP_G0']:.3f} | IA 7d: {(_g7+_l7)} ops • {acc7*100:.2f}%")
    ...
    return ("🩺 <b>Saúde do Guardião</b>\n"
            ...
            f"📊 Placar (hoje): G0=<b>{g0}</b> | G1=<b>{g1}</b> | G2=<b>{g2}</b> | Loss=<b>{loss}</b> | ✅ {acc:.2f}% | 🔥 {streak}\n"
            f"🤖 IA: G0=<b>{ia_g0}</b> | G1=<b>{ia_g1}</b> | G2=<b>{ia_g2}</b> | Loss=<b>{ia_loss}</b> | ✅ {ia_acc:.2f}%\n"
            f"{last_line}\n{phase_line}\n")

async def _health_reporter_task():
    while True:
        try:
            # Envia a cada 30 minutos o placar/saúde
...
async def _daily_reset_task():
    while True:
        try:
            ...
            exec_write("""INSERT OR REPLACE INTO daily_score (yyyymmdd,g0,g1,g2,loss,streak) VALUES (?,0,0,0,0,0)""",(y,))
            exec_write("""INSERT OR REPLACE INTO daily_score_ia (yyyymmdd,g0,g1,g2,loss,streak) VALUES (?,0,0,0,0,0)""",(y,))
            await tg_broadcast("🕛 <b>Reset diário (00:00 UTC)</b> — placares zerados.")
...
# =========================
# Predição (número seco) — bigrama + unigram
# =========================
def ngram_backoff_score(tail: List[int], after_num: Optional[int], candidate: int) -> float:
    ...
    if after_num is not None:
        ...
        ctx1=tail[-1:] if len(tail)>=1 else []
    parts=[]
    if len(ctx2)==2: parts.append((W2, prob_from_ngrams(ctx2[:-1], candidate)))  # bigrama
    if len(ctx1)==1: parts.append((W1, prob_from_ngrams(ctx1[:-1], candidate)))  # unigram
...
def suggest_number(base: List[int], pattern_key: str, strategy: Optional[str], after_num: Optional[int]) -> Tuple[Optional[int],float,int,Dict[int,float]]:
    ...
    # evita empate técnico top1~top2
...
def _ia2_antispam_ok() -> bool:
    eff=_eff(); _ia2_reset_hour(); return IA2_SENT_THIS_HOUR < eff["IA2_MAX_PER_HOUR"]
def _ia2_mark_sent():
...
def _ia_set_post_loss_block():
    global IA2_BLOCK_UNTIL; eff=_eff(); IA2_BLOCK_UNTIL = now_ts() + int(eff["IA2_COOLDOWN_AFTER_LOSS"])
def _has_open_pending() -> bool:
    eff=_eff(); open_cnt=int(_get_scalar("SELECT COUNT(*) FROM pending_outcome WHERE open=1")); return open_cnt >= eff["MAX_CONCURRENT_PENDINGS"]
def _ia2_can_fire_now() -> bool:
    eff=_eff()
    if _ia2_blocked_now(): return False
    if not _ia2_antispam_ok(): return False
    if now_ts() - (IA2_LAST_FIRE_TS or 0) < eff["IA2_MIN_SECONDS_BETWEEN_FIRE"]: return False
    return True
...
def _dyn_thresholds() -> tuple[float, float]:
    """ thresholds dinâmicos: começam em 0%/0.0 e sobem com acc(7d)+amostra """
    eff=_eff()
    g7,l7,acc7=_ia_acc_days(7); samples=_current_samples()
    ...
    min_conf = max(base_conf, eff["MIN_CONF_G0"]); min_gap = max(base_gap, eff["MIN_GAP_G0"])
    min_conf = min(min_conf, 0.68); min_gap = min(min_gap, 0.060)   # caps
    min_conf = max(0.0, min(float(min_conf), CONF_CAP)); min_gap = max(0.0, min(float(min_gap), GAP_CAP))
    ...
    for r in rows:
        ...
        if int(n_observed)==suggested:
            ...
            if stage==0:
                ...
                # converter 1 loss em green do estágio correspondente
                _convert_last_loss_to_green_stage(stage)
                if src=="IA": _convert_last_loss_to_green_ia_stage(stage)
        else:
            # não bateu
            if left>1:
...
# =========================
# IA loop (com EXPLORAÇÃO no warm-up)
# =========================
async def ia2_process_once():
    ...
    # se bloqueado ou antispam, respeita
    eff=_eff()
    ...
    if now_ts() - (IA2_LAST_FIRE_TS or 0) < eff["IA2_MIN_SECONDS_BETWEEN_FIRE"]:
    ...
    # Caso 2: EXPLORAÇÃO durante warm-up (envia mesmo sem confiança)
    samples=_current_samples()
    if samples < WARMUP_SAMPLES and _ia2_can_fire_now():
        # explora pelo mais frequente recente (cauda 40)
        boosts = tail_top2_boost(tail, k=40)
        expl = max(boosts.items(), key=lambda kv: kv[1])[0] if tail else 1
        open_pending(None, expl, source="IA")
        await ia2_send_signal(expl, 0.0, tail_len, "WARMUP")
        _ia2_mark_fire_sent()
        _mark_reason(f"WARMUP_FIRE(expl={expl}, tail={tail_len}, samples={samples})")
        return
...
# =========================
# Relatório Canal x IA
# =========================
def _read_daily_score_days(limit_days:int=7):
    return query_all("""SELECT yyyymmdd, g0, g1, g2, loss, streak FROM daily_score ORDER BY yyyymmdd DESC LIMIT ?""",(limit_days,))
def _read_daily_score_ia_days(limit_days:int=7):
    return query_all("""SELECT yyyymmdd, g0, g1, g2, loss, streak FROM daily_score_ia ORDER BY yyyymmdd DESC LIMIT ?""",(limit_days,))
...
def _mk_relatorio_text(days:int=7) -> str:
    ...
    acc_today=_fmt_pct(g0+g1+g2, g0+g1+g2+loss)
    ria=query_one("SELECT g0,g1,g2,loss,streak FROM daily_score_ia WHERE yyyymmdd=?", (y,))
    ia_g0=(ria["g0"] if ria else 0); ia_g1=(ria["g1"] if ria else 0); ia_g2=(ria["g2"] if ria else 0); ia_loss=(ria["loss"] if ria else 0)
    ia_acc_today=_fmt_pct(ia_g0+ia_g1+ia_g2, ia_g0+ia_g1+ia_g2+ia_loss)
    chan_rows=_read_daily_score_days(days); ia_rows=_read_daily_score_ia_days(days)
    sum_g0=sum((rr["g0"] or 0) for rr in chan_rows)
    sum_g1=sum((rr["g1"] or 0) for rr in chan_rows)
    sum_g2=sum((rr["g2"] or 0) for rr in chan_rows)
    sum_loss=sum((rr["loss"] or 0) for rr in chan_rows)
    sum_ia_g0=sum((rr["g0"] or 0) for rr in ia_rows)
    sum_ia_g1=sum((rr["g1"] or 0) for rr in ia_rows)
    sum_ia_g2=sum((rr["g2"] or 0) for rr in ia_rows)
    sum_ia_loss=sum((rr["loss"] or 0) for rr in ia_rows)
    acc_ndays=_fmt_pct(sum_g0+sum_g1+sum_g2, sum_g0+sum_g1+sum_g2+sum_loss)
    ia_acc_ndays=_fmt_pct(sum_ia_g0+sum_ia_g1+sum_ia_g2, sum_ia_g0+sum_ia_g1+sum_ia_g2+sum_ia_loss)
    return ("📈 <b>Relatório de Desempenho</b>\n"
            f"🗓️ Janela: <b>hoje</b> e últimos <b>{days}</b> dias\n—\n"
            f"📣 Canal (hoje): G0=<b>{g0}</b> | G1=<b>{g1}</b> | G2=<b>{g2}</b> | Loss=<b>{loss}</b> | Acerto=<b>{acc_today:.2f}%</b>\n"
            f"🤖 IA (hoje): G0=<b>{ia_g0}</b> | G1=<b>{ia_g1}</b> | G2=<b>{ia_g2}</b> | Loss=<b>{ia_loss}</b> | Acerto=<b>{ia_acc_today:.2f}%</b>\n—\n"
            f"📣 Canal ({days}d): G0=<b>{sum_g0}</b> | G1=<b>{sum_g1}</b> | G2=<b>{sum_g2}</b> | Loss=<b>{sum_loss}</b> | Acerto=<b>{acc_ndays:.2f}%</b>\n"
            f"🤖 IA ({days}d): G0=<b>{sum_ia_g0}</b> | G1=<b>{sum_ia_g1}</b> | G2=<b>{sum_ia_g2}</b> | Loss=<b>{sum_ia_loss}</b> | Acerto=<b>{ia_acc_ndays:.2f}%</b>\n")
...
async def webhook(token: str, request: Request):
    ...
    # 2) ANALISANDO — só registra sequência, sem sinal
    if is_analise(t):
    ...
    # abre pendência e MANDA para o canal de réplica com máxima velocidade
    ...
    await ia2_process_once()
    return {"ok": True, "sent": True, "number": number, "conf": conf_capped, "samples": samples}
...
async def debug_state():
    try:
        ...
        y=today_key_local()
        r=query_one("SELECT g0,g1,g2,loss,streak FROM daily_score WHERE yyyymmdd=?", (y,))
        g0=(r["g0"] if r else 0); g1=(r["g1"] if r else 0); g2=(r["g2"] if r else 0); loss=(r["loss"] if r else 0); streak=(r["streak"] if r else 0)
        acc=((g0+g1+g2)/max(1,(g0+g1+g2+loss)))
        ria=query_one("SELECT g0,g1,g2,loss,streak FROM daily_score_ia WHERE yyyymmdd=?", (y,))
        ia_g0=(ria["g0"] if ria else 0); ia_g1=(ria["g1"] if ria else 0); ia_g2=(ria["g2"] if ria else 0); ia_loss=(ria["loss"] if ria else 0)
        ia_acc=((ia_g0+ia_g1+ia_g2)/max(1,(ia_g0+ia_g1+ia_g2+ia_loss)))
        hb=_ia2_hour_key()
        ...
        return {
            ...
            "hour_bucket": hb,
            "fires_enviados_nesta_hora": IA2_SENT_THIS_HOUR,
            "cooldown_pos_loss_seconds": int(cooldown_remaining),
            "ultimo_fire_ha_seconds": int(last_fire_age),
            "placar_canal_hoje": {"g0": int(g0), "g1": int(g1), "g2": int(g2), "loss": int(loss), "acc_pct": round(acc*100, 2), "streak": int(streak)},
            "placar_ia_hoje": {"g0": int(ia_g0), "g1": int(ia_g1), "g2": int(ia_g2), "loss": int(ia_loss), "acc_pct": round(ia_acc*100, 2)},
            "ia_acc_7d": {"greens": g7, "losses": l7, "acc_pct": round(acc7*100, 2)},
            "config": {
                "PHASE": eff["PHASE"], "MIN_SAMPLES": MIN_SAMPLES,
                "MIN_CONF_G0": eff["MIN_CONF_G0"], "MIN_GAP_G0": eff["MIN_GAP_G0"],
...
async def debug_flush(request: Request, days: int = 7, key: str = ""):
    ...
    try:
        ...
        except Exception as e: print(f"[FLUSH] relatorio erro: {e}")
        _last_flush_ts=now; return {"ok": True, "flushed": True, "days": max(1,min(days,30))}
    except Exception as e: return {"ok": False, "error": str(e)}
