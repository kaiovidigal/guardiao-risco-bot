...
- **NOVIDADE**: Limite de sinais por janela (madrugada/dia/tarde/noite) via ENV
...
ENV opcionais:    TARGET_CHANNEL, SOURCE_CHANNEL, DB_PATH, DEBUG_MSG, BYPASS_SOURCE
                  ...
                  TZ_NAME
                  # NOVAS (quota por janela):
                  QUOTA_ENABLED, QUOTA_PER_WINDOW,
                  WIN_MADRUGADA, WIN_DIA, WIN_TARDE, WIN_NOITE
...
# ======== Quotas por janela (opcional) ========
QUOTA_ENABLED    = os.getenv("QUOTA_ENABLED", "1").strip() in ("1","true","True","yes","YES")
...
# ========= App =========
app = FastAPI(title="guardiao-auto-bot (GEN webhook)", version="4.4.1+quota")
...
def migrate_db():
    ...
    # === NOVO: tabela de quota por janela ===
...
def _quota_inc(ymd: str, win: str, inc: int=1):
    _exec_write(
        "INSERT INTO quota(ymd,win,used) VALUES(?,?,?) "
        "ON CONFLICT(ymd,win) DO UPDATE SET used=used+?",
        (ymd, win, inc, inc)
    )
...
async def health():
    ...
    # também retorna o uso da quota do dia
    ymd = tz_today_ymd()
    quota = {w: _quota_used(ymd, w) for w in ("MADRUGADA","DIA","TARDE","NOITE")}
    quota["limit"] = QUOTA_PER_WINDOW if QUOTA_ENABLED else None
    return {"ok": True, "db": DB_PATH, "pending_open": pend_open, "pending_seen": seen, "time": ts_str(), "tz": TZ_NAME, "quota": quota}
...
async def webhook(token: str, request: Request):
    ...
    # --- Limite por janela (apenas para novas ENTRADAS) ---
    if QUOTA_ENABLED:
        ymd = tz_today_ymd()
        win = current_window()
        ...
        if used >= QUOTA_PER_WINDOW:
            if DEBUG_MSG:
                await tg_send_text(TARGET_CHANNEL,
                    f"⚠️ Limite de {QUOTA_PER_WINDOW} sinais atingido para {win} ({ymd}).")
    ...
    # abriu mesmo → consome 1 da janela
    if QUOTA_ENABLED:
        _quota_inc(tz_today_ymd(), current_window(), 1)
