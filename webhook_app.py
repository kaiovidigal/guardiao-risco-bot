...
#   GET  /                          (ping)
#   GET  /ping_ia                   (envia mensagem de teste para IA_CHANNEL)
#
# ENV obrigatórias:
...
#   IA_CHANNEL (padrão usa PUBLIC_CHANNEL; recomenda-se setar um canal secundário)
#
...
# Canal secundário da IA
IA_CHANNEL = os.getenv("IA_CHANNEL", PUBLIC_CHANNEL).strip()
...
app = FastAPI(title="Fantan Guardião — FIRE-only (G0 + Recuperação oculta) [FAST+FREE]",
              version="3.11.0-freefire")
...
async def tg_ia(text: str, parse: str="HTML"):
    to = IA_CHANNEL if IA_CHANNEL else PUBLIC_CHANNEL
    await tg_send_text(to, text, parse)
...
async def ping_ia():
    try:
        await tg_ia("🔧 Teste IA: 🚀 Teste mensagem da IA")
...
async def webhook(token: str, request: Request):
    ...
    await tg_broadcast(out)            # canal principal / réplica
    await tg_ia(out)                   # canal IA
...
async def debug_flush(request: Request, days: int = 7, key: str = ""):
    ...
    try:
        if not key or key != os.getenv("FLUSH_KEY", "meusegredo123").strip():
