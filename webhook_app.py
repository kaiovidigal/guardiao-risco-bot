...
# -*- coding: utf-8 -*-
"""
webhook_app.py — v4.5.0 (consolidado)
...
- SAFE_TARGET_MODE para forçar envio sempre ao TARGET_CHANNEL
- Broadcast de chamada Premium a cada N minutos
- Limite de 15 sinais por janela (madrugada/dia/tarde/noite), reset diário
...
# ===== Broadcast =====
BROADCAST_ENABLED    = os.getenv("BROADCAST_ENABLED", "1").strip() in ("1","true","True","yes","YES")
BROADCAST_EVERY_MIN  = int(os.getenv("BROADCAST_EVERY_MIN", "30"))
BROADCAST_MSGS = [
    "🔥 Grupo Premium aberto! Quer receber os sinais antes de todo mundo? **Chame o Adm na descrição** e garanta sua vaga.",
    "⚡ Upgrade de nível! Entre para o **Grupo Premium** e receba entradas filtradas. **Fale com o Adm (link na descrição)** agora.",
    "🎯 Mais assertividade, menos ruído. O **Premium** tem curadoria + prioridade. **Procure o Adm na descrição** e vem com a gente.",
    "🚀 Modo avançado ON! Vagas no **Premium** liberadas. **Chame o Adm na descrição** para entrar.",
    "🔒 Conteúdo exclusivo liberado! Acesso ao **Grupo Premium** via **Adm na descrição**. Garanta o seu agora.",
    "💡 Quer prioridade nos sinais? O **Premium** é pra você. **Fale com o Adm (descrição do grupo)** e faça parte.",
]

# ===== Limite por janela =====
QUOTA_PER_WINDOW = int(os.getenv("QUOTA_PER_WINDOW", "15"))
...
def current_window(dt: Optional[datetime]=None) -> str:
    if dt is None: dt = datetime.now(_TZ)
    ...
    def _in(rg):
        ...
        t2 = h2*60+m2
        return t1 <= t <= t2
    for name,rg in _RANGES.items():
        if _in(rg): return name
    return "DIA"
...
def _quota_inc(ymd: str, win: str, inc: int=1):
    with _tx() as con:
        cur = con.execute("SELECT used FROM quota WHERE ymd=? AND win=?", (ymd, win)).fetchone()
        if cur:
            con.execute("UPDATE quota SET used=used+? WHERE ymd=? AND win=?", (inc, ymd, win))
        else:
            con.execute("INSERT INTO quota (ymd, win, used) VALUES (?,?,?)",
                        (ymd, win, int(inc)))

def _quota_can_post(ymd: str, win: str) -> bool:
    return _quota_used(ymd, win) < QUOTA_PER_WINDOW

def _quota_reset_for_today(today: str):
    # sem carry-over: novo ymd começa do zero automaticamente
    return
...
def check_and_maybe_reset_score():
    ...
    if last != today:
        reset_score()
        _quota_reset_for_today(today)  # também vira a cota pelo novo ymd
...
async def tg_send_text(chat_id: str, text: str, parse: str="HTML"):
    ...
    # 🔒 força sempre enviar para o canal alvo, se habilitado
    if SAFE_TARGET_MODE:
...
async def _broadcast_loop():
    if not BROADCAST_ENABLED:
        return
    cycler = itertools.cycle(BROADCAST_MSGS)
    try:
        while True:
            msg = next(cycler)
            try:
                # usa Markdown para **negrito** nas mensagens
                await tg_send_text(TARGET_CHANNEL, msg, parse="Markdown")
            except Exception:
                pass
            await asyncio.sleep(max(60, BROADCAST_EVERY_MIN * 60))
...
async def webhook(token: str, request: Request):
    ...
    # Contabiliza cota após abrir com sucesso
