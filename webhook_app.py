...
- (NOVO) G5: fecha só após 6 observações (G0..G5)
- (NOVO) Copier especializado 30–40 min: mínimo 30min entre sinais; timeout 40min
- (NOVO) Novo sinal só após fechamento do G5
...
# ---- Gales / observações ----
GALES_MAX     = 5                      # até G5
OBS_REQUIRED  = GALES_MAX + 1          # G0..G5 = 6 observações

# ======== Copier especializado 30–40 min ========
MIN_BETWEEN_SIGNALS_MIN = int(os.getenv("MIN_BETWEEN_SIGNALS_MIN", "30"))  # 30 min mínimo
MAX_SIGNAL_WINDOW_MIN   = int(os.getenv("MAX_SIGNAL_WINDOW_MIN", "40"))    # 40 min timeout
OBS_TIMEOUT_SEC         = MAX_SIGNAL_WINDOW_MIN * 60

# ======== Gates (não bloqueiam abertura; mantidos p/ debug/futuro) ========
CONF_MIN    = 0.62
GAP_MIN     = 0.06
H_MAX       = 0.95
FREQ_WINDOW = 80
...
def migrate_db():
    ...
    if not row:
        cur.execute("INSERT INTO state (id, cooldown_left, loss_streak, last_reset_ymd, last_close_ts) VALUES (1,0,0,'',0)")
...
def _is_processed(update_id: str) -> bool:
    ...
    con = _connect()
    row = con.execute("SELECT 1 FROM processed WHERE update_id=?", (update_id)).fetchone() if isinstance(update_id, tuple) else con.execute("SELECT 1 FROM processed WHERE update_id=?", (update_id,)).fetchone()
    con.close()
    return bool(row)
...
def _get_last_close_ts() -> int:
    con = _connect()
    row = con.execute("SELECT last_close_ts FROM state WHERE id=1").fetchone()
    con.close()
    return int((row["last_close_ts"] if row else 0) or 0)

def _set_last_close_ts(ts:int):
    _exec_write("UPDATE state SET last_close_ts=? WHERE id=1", (int(ts),))
...
GALE_RXS = {
    1: re.compile(r"Estamos\s+no\s*1[ºo]\s*gale", re.I),
    2: re.compile(r"Estamos\s+no\s*2[ºo]\s*gale", re.I),
    3: re.compile(r"Estamos\s+no\s*3[ºo]\s*gale", re.I),
    4: re.compile(r"Estamos\s+no\s*4[ºo]\s*gale", re.I),
    5: re.compile(r"Estamos\s+no\s*5[ºo]\s*gale", re.I),
}
...
def parse_close_numbers(text: str, limit:int=OBS_REQUIRED) -> List[int]:
    t = _normalize_keycaps(re.sub(r"\s+", " ", text))
    groups = PAREN_GROUP_RX.findall(t)
    if groups:
        last = groups[-1]
        nums = re.findall(r"[1-4]", _normalize_keycaps(last))
        return [int(x) for x in nums][:limit]
...
def _seen_append(row: sqlite3.Row, new_items: List[str]):
    ...
    for it in new_items:
        if len(cur_seen) >= OBS_REQUIRED: break
        if it not in cur_seen:
            cur_seen.append(it)
    seen_txt = "-".join(cur_seen[:OBS_REQUIRED])
    with _tx() as con:
...
def _stage_from_observed(suggested: int, obs: List[int]) -> Tuple[str, str]:
    """Retorna (outcome, 'Gk'), k=0..GALES_MAX; LOSS fecha em G5 (k=5)."""
    if not obs:
        return ("LOSS", f"G{GALES_MAX}")
    for k in range(0, GALES_MAX+1):  # G0..G5
        if len(obs) >= (k+1) and obs[k] == suggested:
            return ("GREEN", f"G{k}")
    return ("LOSS", f"G{GALES_MAX}")
...
def _close_with_outcome(row: sqlite3.Row, outcome: str, final_seen: str, stage_lbl: str, suggested: int):
    ...
    try:
        ...
        if outcome.upper() == "GREEN":
            # pesos por estágio: mais cedo = maior reforço
            stage_index = int(stage_lbl[1:]) if stage_lbl.startswith("G") else GALES_MAX
            stage_weight_map = {0:1.00, 1:0.80, 2:0.65, 3:0.55, 4:0.48, 5:0.40}
            delta = FEED_POS * stage_weight_map.get(stage_index, 0.50)
    ...
    # carimba "último fechamento" p/ respeitar 30min antes do próximo sinal
    try:
        _set_last_close_ts(now_ts())
    except Exception:
        pass
...
def _maybe_close_by_timeout():
    """Após MAX_SIGNAL_WINDOW_MIN: completa com X até OBS_REQUIRED e fecha."""
    ...
    if len(seen_list) < OBS_REQUIRED:
        while len(seen_list) < OBS_REQUIRED:
            seen_list.append("X")
        final_seen = "-".join(seen_list[:OBS_REQUIRED])
...
def close_pending(outcome:str):
    """Força fechar preenchendo X até OBS_REQUIRED (não usada no fluxo normal)."""
    row = get_open_pending()
    if not row: return
    seen_list = _seen_list(row)
    while len(seen_list) < OBS_REQUIRED:
...
def _open_pending_with_ctx(suggested:int, after:Optional[int], ctx1,ctx2,ctx3,ctx4) -> bool:
    """Abertura transacional: só abre se não existir pendência aberta e respeita janela 30min desde último fechamento."""
    with _tx() as con:
        row = con.execute("SELECT 1 FROM pending WHERE open=1 LIMIT 1").fetchone()
        if row:
            return False
        # respeita janela mínima entre sinais
        row2 = con.execute("SELECT last_close_ts FROM state WHERE id=1").fetchone()
        last_close = int((row2["last_close_ts"] if row2 else 0) or 0)
        if last_close > 0 and (now_ts() - last_close) < MIN_BETWEEN_SIGNALS_MIN*60:
            return False
...
async def webhook(token: str, request: Request):
    ...
    # timeout pode fechar pendência antiga (até OBS_REQUIRED observações)
    ...
    if ANALISANDO_RX.search(_normalize_keycaps(text)):
        ...
        if seq:
            ...
            pend = get_open_pending()
            if pend:
                # usar até completar OBS_REQUIRED observados
                to_add = []
                cur_seen = _seen_list(pend)
                need = OBS_REQUIRED - len(cur_seen)
                if need > 0:
                    ...
                    if to_add:
                        ...
                        pend = get_open_pending()
                        cur_seen = _seen_list(pend)
                        if len(cur_seen) >= OBS_REQUIRED:
                            suggested = int(pend["suggested"] or 0)
                            obs_nums = [int(x) for x in cur_seen if x.isdigit()]
                            outcome, stage_lbl = _stage_from_observed(suggested, obs_nums)
                            final_seen = "-".join(cur_seen[:OBS_REQUIRED])
    ...
    # 1) Gales (informativo G1..G5)
    for g, rx in GALE_RXS.items():
        if rx.search(text):
            if get_open_pending():
                set_stage(g)
    ...
    if GREEN_RX.search(text) or LOSS_RX.search(text):
        pend = get_open_pending()
        if pend:
            nums = parse_close_numbers(text, limit=OBS_REQUIRED)  # pode ter 1..OBS_REQUIRED
            if nums:
                _seen_append(pend, [str(n) for n in nums])
                pend = get_open_pending()
            seen_list = _seen_list(pend) if pend else []
            if pend and len(seen_list) >= OBS_REQUIRED:
                suggested = int(pend["suggested"] or 0)
                obs_nums = [int(x) for x in seen_list if x.isdigit()]
                outcome, stage_lbl = _stage_from_observed(suggested, obs_nums)
                final_seen = "-".join(seen_list[:OBS_REQUIRED])
    ...
    # se existir pendência e estiver incompleta, NÃO abre novo (espera fechar G5)
    pend = get_open_pending()
    if pend:
    ...
    # Respeita janela mínima (30min) antes de abrir novo sinal
    last_close = _get_last_close_ts()
    if last_close > 0 and (now_ts() - last_close) < MIN_BETWEEN_SIGNALS_MIN*60:
        if DEBUG_MSG:
            await tg_send_text(TARGET_CHANNEL, "DEBUG: Janela mínima de 30min ainda não cumprida — não abri novo sinal.")
        return {"ok": True, "skipped": "janela_minima_30m"}
    ...
    if not opened:
        if DEBUG_MSG:
            await tg_send_text(TARGET_CHANNEL, "DEBUG: Já existe pending open ou janela mínima não cumprida — não abri novo.")
        return {"ok": True, "skipped": "pending_already_open_or_window"}
