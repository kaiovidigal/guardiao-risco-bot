...
- (NOVO) Timeout desativado e proteção anti-repetição de sugestão
...
def migrate_db():
    ...
    # --- NOVO: coluna para evitar repetição imediata
    try:
        cur.execute("ALTER TABLE state ADD COLUMN last_suggested INTEGER DEFAULT -1")
...
# NOVO: last_suggested (evitar repetição)
def _get_last_suggested() -> int:
    con = _connect()
    row = con.execute("SELECT last_suggested FROM state WHERE id=1").fetchone()
    con.close()
    try:
        return int(row["last_suggested"])
    except Exception:
        return -1

def _set_last_suggested(v:int):
    _exec_write("UPDATE state SET last_suggested=? WHERE id=1", (int(v),))
...
def _prob_post_from_tail(tail: List[int], after: Optional[int]) -> Dict[int, float]:
    # wrapper do _prob_from_ngrams/_feedback_prob
    cands = [1,2,3,4]
...
def _close_with_outcome(row: sqlite3.Row, outcome: str, final_seen: str, stage_lbl: str, suggested: int):
    ...
    try:
        ...
        if true_first is not None:
            ...
            post_e1 = _prob_post_from_tail(tail_now, after=None)
...
def _maybe_close_by_timeout():
    # com OBS_TIMEOUT_SEC gigante, isso só roda em casos extremos;
    # mantido por compatibilidade (não fecha com -1-2-3 no dia-a-dia)
...
async def health():
    ...
    return {"ok": True, "db": DB_PATH, "pending_open": pend_open, "pending_seen": seen, "time": int(time.time()), "tz": TZ_NAME}
...
async def webhook(token: str, request: Request):
    ...
    # timeout (praticamente desativado)
    ...
    # grava referência do último sugerido
...
# ========= Context helpers (usadas em abertura) =========
