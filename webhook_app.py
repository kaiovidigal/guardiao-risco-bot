...
app = FastAPI(title="Guardião — canais separados", version="3.12.0")
...
def bump_pattern(pattern_key: str, number: int, won: bool):
    ...
    exec_write("""      INSERT INTO stats_pattern (pattern_key, number, wins, losses)
      VALUES (?,?,?,?)
...
def bump_strategy(strategy: str, number: int, won: bool):
    ...
    exec_write("""      INSERT INTO stats_strategy (strategy, number, wins, losses)
      VALUES (?,?,?,?)
...
def ngram_backoff_score(tail: List[int], after_num: Optional[int], candidate: int) -> float:
    ...
    parts = []
    if len(ctx4)==4: parts.append((0.38, prob_from_ngrams(ctx4[:-1], candidate)))
    if len(ctx3)==3: parts.append((0.30, prob_from_ngrams(ctx3[:-1], candidate)))
    if len(ctx2)==2: parts.append((0.20, prob_from_ngrams(ctx2[:-1], candidate)))
    if len(ctx1)==1: parts.append((0.12, prob_from_ngrams(ctx1[:-1], candidate)))
...
def suggest_number(base: List[int], pattern_key: str, strategy: Optional[str], after_num: Optional[int]) -> Tuple[Optional[int],float,int,Dict[int,float]]:
    ...
    for c in base:
        ...
        prior = 1.0/len(base)
        score = (prior) * ((ng or 1e-6) ** 1.05) * (p_pat ** 0.70) * (p_str ** 0.40) * boost_pat * boost_hist
        scores[c] = score
...
def _ia2_antispam_ok() -> bool:
    _ia2_reset_hour(); return _ia2_sent_this_hour < IA2_MAX_PER_HOUR
def _ia2_mark_sent():
...
async def close_pending_with_result(n_observed: int, event_kind: str):
    try:
        rows = query_all("""            SELECT id, created_at, strategy, suggested, stage, open, window_left,
