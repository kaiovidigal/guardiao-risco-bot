...
SCORE_G0_ONLY = True      # placar "simples" do dia só conta G0 e Loss (limpo)
...
# Detecção robusta de GREEN/RED (vários formatos) — pega o ÚLTIMO número do parênteses
GREEN_PATTERNS = [
    re.compile(r"APOSTA\s+ENCERRADA.*?\bGREEN\b.*?\(([^)]*)\)", re.I | re.S),
    re.compile(r"\bGREEN\b.*?Número[:\s]*\(([^)]*)\)", re.I | re.S),
    re.compile(r"\bGREEN\b.*?\(([^)]*)\)", re.I | re.S),
]
RED_PATTERNS = [
    re.compile(r"APOSTA\s+ENCERRADA.*?\bRED\b.*?\(([^)]*)\)", re.I | re.S),
    re.compile(r"\bLOSS\b.*?Número[:\s]*\(([^)]*)\)", re.I | re.S),
    re.compile(r"\bRED\b.*?\(([^)]*)\)", re.I | re.S),
]

def _last_digit_inside(group_text: str) -> Optional[int]:
    nums = re.findall(r"[1-4]", group_text)
...
def extract_green_number(text: str) -> Optional[int]:
    ...
    for rx in GREEN_PATTERNS:
        m = rx.search(t)
        if m:
            return _last_digit_inside(m.group(1))
    return None
...
def update_daily_score(stage: Optional[int], won: bool):
    ...
    if SCORE_G0_ONLY:
        if won and stage == 0:
            ...
            loss += 1
            streak = 0
        # Recuperações (G1/G2) NÃO entram neste placar "simples"
    else:
        if won:
            if stage == 0: g0 += 1
            elif stage == 1: g1 += 1
...
def add_recovery_green(stage:int, is_ia:bool):
    """Acumula G1/G2 para relatório de 30min; mantém placar simples (G0/Loss) intacto."""
    stage = int(stage)
    y = today_key_local()
    row = query_one("SELECT g0,g1,g2,loss,streak FROM daily_score WHERE yyyymmdd=?", (y,))
    if not row:
    ...
    if is_ia:
        # IA: manter placar simples e acumular em IA diário se desejar (aqui só mantemos G0/Loss no simples)
        pass
...
def _health_text() -> str:
    ...
    g0, g1, g2, loss, acc = _daily_score_snapshot()
    ...
    return (
        ...
        f"🪄 Recuperação (hoje): G1=<b>{g1}</b> | G2=<b>{g2}</b>\n"
        f"✅ Acerto simples: <b>{acc:.2f}%</b>\n"
        "—\n"
        f"🤖 <b>IA</b> G0=<b>{ia_g0}</b> | Loss=<b>{ia_loss}</b>\n"
        ...
        for r in rows:
            ...
            if int(n_observed) == suggested:
                ...
                if stage == 0:
                    ...
                    # Recuperação oculta: contabiliza G1/G2 para relatório + converte último loss do placar simples
                    add_recovery_green(stage, is_ia=(src=="IA"))
...
def _mk_relatorio_text(days:int=7) -> str:
    ...
    g1 = (r["g1"] if r else 0); g2 = (r["g2"] if r else 0)
    ...
    return (
        ...
        "—\n"
        f"📣 <b>Canal (hoje)</b>: G0=<b>{g0}</b> | G1=<b>{g1}</b> | G2=<b>{g2}</b> | Loss=<b>{loss}</b> | Acerto G0=<b>{acc_today:.2f}%</b>\n"
        f"🤖 <b>IA (hoje)</b>: G0=<b>{ia_g0}</b> | Loss=<b>{ia_loss}</b> | Acerto=<b>{ia_acc_today:.2f}%</b>\n"
        "—\n"
        f"📣 <b>Canal ({days}d)</b>: G0=<b>{sum_g0}</b> | G1=<b>{sum_g1}</b> | G2=<b>{sum_g2}</b> | Loss=<b>{sum_loss}</b> | Acerto G0=<b>{acc_ndays:.2f}%</b>\n"
...
async def debug_state():
    try:
        ...
        return {
            ...
            "placar_canal_hoje": {"g0": int(g0), "g1": int(g1), "g2": int(g2), "loss": int(loss), "acc_pct": round(acc*100, 2)},
